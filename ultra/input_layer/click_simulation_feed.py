...
class ClickSimulationFeed(BaseInputFeed):
    ...
    def prepare_sim_clicks_with_index(
            self, data_set, index, docid_inputs, letor_features, labels, check_validation=True, get_all_doc=False):
        if get_all_doc:
            rank_list_size = self.max_candidate_num
        else:
            rank_list_size = self.rank_list_size
        i = index
        ...
        docid_inputs.append(list([-1 if data_set.initial_list[i][x]
                                  < 0 else base + x for x in range(rank_list_size)]))
        labels.append(click_list)

    def get_batch(self, data_set, check_validation=False, data_format = "ULTRA", get_all_doc=False):
        ...
        while len(docid_inputs) < self.batch_size:
            ...
            self.prepare_sim_clicks_with_index(data_set, i,
                                               docid_inputs, letor_features, labels, check_validation, get_all_doc)
        ...
        batch_docid_inputs = []
        batch_labels = []
        for length_idx in range(rank_list_size):
